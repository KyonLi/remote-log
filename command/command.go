...
func (cmd *Command) Execute(output chan Message) {
	...
	client := &ssh.Client{
		...
		PrivateKey: cmd.Server.PrivateKey,
	}
	...
	go bindOutput(cmd.Host, output, &cmd.Stdout, "")
	go bindOutput(cmd.Host, output, &cmd.Stderr, "Error:")
...
func bindOutput(host string, output chan Message, input *io.Reader, prefix string) {
